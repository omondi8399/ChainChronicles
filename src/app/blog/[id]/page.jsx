...
const BlogDetails = (ctx) => {
  ...
  useEffect(() => {
    async function fetchComments() {
      const res = await fetch(
        `https://chain-chroniclez.vercel.app/api/comment/${ctx.params.id}`,
        { cache: "no-store" }
      );
  ...
  useEffect(() => {
    async function fetchBlog() {
      const res = await fetch(
        `https://chain-chroniclez.vercel.app/api/blog/${ctx.params.id}`,
        { cache: "no-store" }
      );
  ...
  const handleLike = async () => {
    try {
      const res = await fetch(
        `https://chain-chroniclez.vercel.app/api/blog/${ctx.params.id}/like`,
        {
          headers: {
            Authorization: `Bearer ${session?.user?.accessToken}`,
          },
  ...
  const handleComment = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `https://chain-chroniclez.vercel.app/api/comment`,
        {
          headers: {
